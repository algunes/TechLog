...
public class UserDomainPermissionsFilter implements Filter {


    public UserDomainPermissionsFilter() {
    }
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		boolean isAdmin = (sessionUser != null ? 
				sessionUser.getDomainPermissions().getUserDomain().is_create() 
				&& sessionUser.getDomainPermissions().getUserDomain().is_read()
				&& sessionUser.getDomainPermissions().getUserDomain().is_update()
				&& sessionUser.getDomainPermissions().getUserDomain().is_delete()
				: false);
	...
	if (!isAdmin && !isSelfSubject) {
		...
		case "updateUserPermissions" : {
			...
			if (!isAdmin) {
				req.setAttribute("alert", "You are not Authorized to change the User Permissions!");
				req.setAttribute("user", user);
				RequestDispatcher rd = req.getRequestDispatcher("ShowUser.jsp");
				rd.forward(request, response);
			}
		...
		case "removeUser" : {
			...
			if (isAdmin) {
				if (!(isAdmin && isSelfSubject)) {
					...
					req.setAttribute("alert", "Are you kidding?"); // trying to delete yourself!!
					req.setAttribute("user", user);
					RequestDispatcher rd = req.getRequestDispatcher("ShowUser.jsp");
					rd.forward(request, response);
					break;
